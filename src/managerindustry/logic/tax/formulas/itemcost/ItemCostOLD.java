...
//    private float jobFee = 0f;
    ...
    public ItemCostOLD(Map<String, ReportCalculatedComponentX>  totalCalculatedComponentXMap, 
...
//        this.systemCostIndex = SystemCostIndex.SystemCostIndex(solarSystemID, actvity);
    ...
    public float getJobInstallationCost(int run){   
        return baseJobCost * systemCostIndex * run * 1.1f;
    }
    ...
    public float getFacilityTaxesPerItem(float taxRate){
        ...
        for (Float sumOfEachJobcost : sumOfEachJobcosts) {
            System.out.println("DBG XXX " + String.format("%.0f", sumOfEachJobcost * taxRate / 100 * 1.1f));
            jobfee += ( sumOfEachJobcost * systemCostIndex * 1.1f ) * taxRate / 100 ;
        }
        return jobfee;
    }
